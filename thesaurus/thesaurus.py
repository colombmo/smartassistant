...
def fetchWordData(inputWord):
    ...
    for d in reversed(data):
        if "window.INITIAL_STATE" in str(d):
            data = str(d).replace("<script>","").replace("window.INITIAL_STATE = ", "").replace(";","").replace("</script>","").replace('undefined', '""') # remove 'window.INITIAL_STATE = ', ';', '<script>' and '</script>'
    ...
    calc_relevance = lambda x: 1 if x == 0 or abs(x) == 10 else 2 if abs(x) == 50 else 3 #[None, 10, 50, 100].index(x)
